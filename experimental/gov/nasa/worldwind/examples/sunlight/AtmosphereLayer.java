 ...
 * @version $Id: AtmosphereLayer.java 14977 2011-03-16 22:28:55Z tgaskins $
 */
public class AtmosphereLayer extends AbstractLayer
{
    ...
    public void doRender(DrawContext dc)
    {
        ...
        try
        {
            ...
            if (this.update || dlId == null || Math.abs(this.lastRebuildHorizon - tangentalDistance) > 100)
            {
                dlId = this.makeSkyDome(dc, (float) (domeRadius), horizonLat, zenithLat, SLICES, STACKS);
                this.lastRebuildHorizon = tangentalDistance;
                this.update = false;
            }

            ogsh.pushAttrib(gl, GL.GL_COLOR_BUFFER_BIT | GL.GL_DEPTH_BUFFER_BIT | GL.GL_TRANSFORM_BIT
                | GL.GL_POLYGON_BIT | GL.GL_ENABLE_BIT
                | GL.GL_CURRENT_BIT);
     ...
     *
     * @return the display list resource ID
     */
    protected int[] makeSkyDome(DrawContext dc, float radius, double startLat, double endLat, int slices, int stacks)
    {
    ...
    protected Matrix computeSkyTransform(DrawContext dc)
    {
        ...
        transform = transform.multiply(
            dc.getGlobe().computeSurfaceOrientationAtPosition(dc.getView().getEyePosition()));
