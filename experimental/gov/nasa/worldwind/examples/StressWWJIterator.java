    ...
    private void nextViewState(long timeToMove)
    {
            ...
            && wwd.getModel().getGlobe() != null)
        {
            ...
            else
            {
                ...
                long cap = new Double(wwd.getGpuResourceCache().getCapacity() * .95).longValue();
                if ( cap < minCacheCap)
                    wwd.getGpuResourceCache().setCapacity(maxCacheCap);
                else
                    wwd.getGpuResourceCache().setCapacity(cap);

                cap = new Double(wwd.getGpuResourceCache().getLowWater() * .95).longValue();
                if ( cap < minCacheLowCap)
                    wwd.getGpuResourceCache().setLowWater(maxCacheLowCap);
                else
                    wwd.getGpuResourceCache().setLowWater(cap);
