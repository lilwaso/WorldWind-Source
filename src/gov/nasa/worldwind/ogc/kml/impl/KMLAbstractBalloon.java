 ...
 *
 * @author pabercrombie
 * @version $Id: KMLAbstractBalloon.java 15205 2011-04-05 18:25:36Z pabercrombie $
 */
public abstract class KMLAbstractBalloon implements Balloon, WebResourceResolver, PropertyChangeListener
{
     ...
     * {@inheritDoc}
     * <p/>
     * Overridden to adjust the balloon text based on the highlight state.
     *
    ...
    public void setHighlighted(boolean highlighted)
    {
        ...
        // Set the text based on the highlight state. Only set the text if it actually changed.
    ...
    protected void makeAttributesCurrent(String attrType)
    {
        ...
        if (KMLConstants.NORMAL.equals(attrType))
        {
            ...
            {
                this.getBalloon().setText(this.createDefaultBalloonText());
    ...
    public URL resolve(String address)
    {
        ...
        try
        {
            // BrowserBalloons on Mac OS X represent the address as relative path with a leading slash "/". The KMLDoc
            // expects the address to start with a folder name or a file name relative to the KML file's location, and
            // without any leading slash. Since we know the address represents a relative path, we strip the leading
            // slash to ensure that the KMLDoc recognizes the address as a relative path.
            String relativePath = WWIO.stripLeadingSeparator(address);
            ...
            String absolutePath = this.parent.getRoot().getKMLDoc().getSupportFilePath(relativePath);
            if (!WWUtil.isEmpty(absolutePath))
                return WWIO.makeURL(absolutePath, "file");
        }
        catch (IOException e)
        {
