 ...
 *
 * @author tag
 * @version $Id: KMLRoot.java 15470 2011-05-20 01:17:19Z tgaskins $
 */
public class KMLRoot extends KMLAbstractObject implements KMLRenderable
{
    ...
    /** Controller to refresh <code>{@link KMLLink}</code> instances based on each link's refresh mode. */
    protected KMLLinkController linkController;
    /**
    ...
    /**
     * Invoked during {@link #initialize(boolean)} to create the KML Link controller.
     *
     * @return a new link controller.
     */
    protected KMLLinkController createLinkController()
    {
        return new KMLLinkController();
    }
    ...
    public Object resolveLocalReference(String linkBase, String linkRef)
    {
        ...
        try
        {
            ...
            if (!file.exists() || !WWIO.isContentType(file, KMLConstants.KML_MIME_TYPE))
                return linkBase;
        ...
        throws IOException, XMLStreamException
    {
        ...
        if (KMLConstants.KML_MIME_TYPE.equals(contentType))
            kmlDoc = new KMLInputStream(refStream, WWIO.makeURI(linkBase));
        else if (KMLConstants.KMZ_MIME_TYPE.equals(contentType))
    ...
    /**
     * Returns the controller that refreshes {@link KMLLink} elements
     *
     * @return The KML Link controller.
     */
    public KMLLinkController getLinkController()
    {
