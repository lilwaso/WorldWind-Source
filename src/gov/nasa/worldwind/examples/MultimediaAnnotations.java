 ...
 * @version $Id: MultimediaAnnotations.java 14890 2011-03-05 00:56:46Z dcollins $
 */
public class MultimediaAnnotations extends ApplicationTemplate
{
    public static class AppFrame extends ApplicationTemplate.AppFrame
    {
        private final static String HELLO_AUDIO_FILE_PATH
            = "http://simplythebest.net/sounds/WAV/events_WAV/event_WAV_files/welcome.wav";
        ...
        private final static String MSH_IMAGE_FILE_PATH
            = "http://worldwind.arc.nasa.gov/java/demos/Images/MountStHelens_01_800.jpg";
        private final static String MSH_IMAGE_FILE_PATH1
                            ...
                            if (event.getTopObject() instanceof Annotation)
                            {
                                ...
                                if (po.getValue(AVKey.TEXT) != null)
                                {
                                    ...
                                    if (url != null && url.equals(PLAY_AUDIO_CMD))
                                        playAnnotationAudio((GlobeAnnotation) event.getTopObject(),
                                            event.getPickPoint());
                                        // Check for image play url command
                                    else if (url != null && url.equals(PLAY_IMAGE_CMD))
                                        playAnnotationImage((GlobeAnnotation) event.getTopObject(),
                                            event.getPickPoint());
                                        // Check for slides play command
                                    else if (url != null && url.equals(PLAY_SLIDES_CMD))
                                        playAnnotationSlides((GlobeAnnotation) event.getTopObject(),
                                            event.getPickPoint());
                                ...
                                if (AppFrame.this.currentAnnotation != event.getTopObject())
                                {
                                    ...
                                    AppFrame.this.savedBorderColor
                                        = AppFrame.this.currentAnnotation.getAttributes().getBorderColor();
