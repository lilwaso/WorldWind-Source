...
/**
 * This example demonstrates how to install elevation data into a World Wind {@link
 ...
 * to place the imported elevations. This example uses the FileStore's install location. </li> <li>Compute a unique
 * cache name for the elevations. In this example, the cache name is "Examples/ElevationsName", where
 * "ElevationsName" is the elevation data's display name, stripped of any illegal filename characters.</li> <li>Import
 * the elevations by constructing, configuring and running a {@link gov.nasa.worldwind.data.TiledElevationProducer}.</li>
 * <li>The elevation data is subsequently described by a configuration {@link org.w3c.dom.Document}, which we use to
 * construct an ElevationModel via the {@link gov.nasa.worldwind.Factory} method {@link
 * gov.nasa.worldwind.Factory#createFromConfigSource(Object, gov.nasa.worldwind.avlist.AVList)}.</li> </ol>
 *
 * @author tag
 * @version $Id: InstallElevations.java 14331 2010-12-28 19:39:55Z tgaskins $
 */
public class InstallElevations extends ApplicationTemplate
{
    ...
    // This example's elevations file is downloaded from the following location.
    ...
    public static class AppFrame extends ApplicationTemplate.AppFrame
    {
        ...
        protected void installElevations()
        {
            ...
            final ElevationModel em = importElevations("Snohomish WA LiDAR DTM", sourceFile, fileStore);
            if (em == null)
                return;
            ...
            SwingUtilities.invokeLater(new Runnable()
            {
                public void run()
                {
                    ...
                    // Set the view to look at the imported elevations. Get the location from the elevation model's
                    // construction parameters.

                    AVList params = (AVList) em.getValue(AVKey.CONSTRUCTION_PARAMETERS);
                    Sector sector = (Sector) params.getValue(AVKey.SECTOR);
                    AppFrame.this.getWwd().getView().goTo(new Position(sector.getCentroid(), 0), 50e3);
                }
            });
        }

        protected ElevationModel importElevations(String displayName, Object elevationSource, FileStore fileStore)
        {
            ...
            // Create a unique cache name that specifies the FileStore path to the imported elevations.
            ...
            // Extract the data configuration document from the production results. If production successfully completed,
            // the TiledElevationProducer should always contain a document in the production results, but test the
            // results anyway.
