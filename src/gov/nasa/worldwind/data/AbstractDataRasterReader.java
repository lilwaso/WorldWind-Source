...
/**
 * Abstract base class for most {@link DataRasterReader} implemenetations.
 *
 * @author dcollins
 * @version $Id: AbstractDataRasterReader.java 14363 2011-01-04 06:35:06Z garakl $
 */
public abstract class AbstractDataRasterReader extends AVListImpl implements DataRasterReader
{
    ...
    {
        if (DataImportUtil.isImagery(params))
            return true;

        try
        {
            AVList metadata = this.readMetadata(source, params);
            return DataImportUtil.isImagery(metadata);
        }
        catch (IOException e)
        {
            return false;
    ...
    public boolean isElevationsRaster(Object source, AVList params)
    {
        ...
        {
            AVList metadata = this.readMetadata(source, params);
            return DataImportUtil.isElevations(metadata);
        }
        catch (IOException e)
        {
            return false;
